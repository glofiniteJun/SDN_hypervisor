...
public class OVXFlowManager {
    ...
    public synchronized Integer storeL4FlowValues(final BigInteger dualMacPort, final byte[] srcMac, final byte[] dstMac)
            throws IndexOutOfBoundException {
        // TODO: Optimize flow numbers
        Integer flowId = this.flowValues.inverse().get(dualMacPort);
        if (flowId == null) {
            ...
            this.flowValues.put(flowId, dualMacPort);
        }
        return flowId;
    }
    
    public synchronized Integer storeL3FlowValues(final BigInteger dualMacIP, final byte[] srcMac, final byte[] dstMac)
            throws IndexOutOfBoundException {
        // TODO: Optimize flow numbers
        Integer flowId = this.flowValues.inverse().get(dualMacIP);
        if (flowId == null) {
            flowId = this.flowCounter.getNewIndex();
            log.debug(
                    "virtual net = {}: save flowId = {} that is associated to {} {}",
                    ...
                    MacAddress.of(srcMac).toString(),
                    MacAddress.of(dstMac).toString());
            this.flowValues.put(flowId, dualMacIP);
        }
        return flowId;
    }
    ...
    public synchronized Integer getL4FlowId(final byte[] srcMac, final byte[] dstMac, final Integer srcPort, final Integer dstPort)
            throws DroppedMessageException, IndexOutOfBoundException {
    	byte[] srcPortBytes = ByteBuffer.allocate(4).putInt(srcPort).array();
    	byte[] dstPortBytes = ByteBuffer.allocate(4).putInt(dstPort).array();
    	byte[] port = ArrayUtils.addAll(srcPortBytes, dstPortBytes);
    	byte[] mac = ArrayUtils.addAll(srcMac, dstMac);
    	
        final BigInteger dualMacPort = new BigInteger(ArrayUtils.addAll(port,
                mac));
                
        final Integer flowId = this.flowValues.inverse().get(dualMacPort);
        if (flowId != null && flowId != 0) {
            ...
            return this.storeL4FlowValues(dualMacPort, srcMac, dstMac);
        }
    }
    
    
    public synchronized Integer getL3FlowId(final byte[] srcMac, final byte[] dstMac, final byte[] srcIP, final byte[] dstIP)
            throws DroppedMessageException, IndexOutOfBoundException {
    	byte[] ip = ArrayUtils.addAll(srcIP, dstIP);
    	byte[] mac = ArrayUtils.addAll(srcMac, dstMac);
    	
        final BigInteger dualMacIP = new BigInteger(ArrayUtils.addAll(ip,
                mac));
        
        final Integer flowId = this.flowValues.inverse().get(dualMacIP);
        log.info("Before creating new flow ID for tenant {}", this.tenantId);
        if (flowId != null && flowId != 0) {
            log.info(
                    "virtual net = {}: retrieving flowId {} that is associated to {} {}",
                    this.tenantId, flowId,
            ...
            // TODO: this is probably incorrect if the match is not identical
            // at both ends of the virtual link
        	log.info("create new flow ID for tenant {}", this.tenantId);
            return this.storeL3FlowValues(dualMacIP, srcMac, dstMac);
        }
    }
